...
describe('processSmartAlerts', () => {
  ...
  test('should report multiple threshold violations', async () => {
    const config = {
      id: 'config-multi',
      agent_id: 'agent-123',
      cpu_threshold: 80,
      ...
      last_triggered_at: null,
      channel: { active: true, type: 'webhook', config: { webhook_url: 'http://example.com' } },
    };

    // Use preloadedConfigs to test the logic directly
    ...
    expect(alertsBuilder.insert).toHaveBeenCalled();
    const callArgs = alertsBuilder.insert.mock.calls[0][0];

    expect(callArgs.message).toContain('CPU usage exceeded');
    expect(callArgs.message).toContain('Memory usage exceeded');
    expect(callArgs.message).not.toContain('Latency exceeded');
  });
});
