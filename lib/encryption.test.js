...
describe("lib/encryption", () => {
  ...
  describe("verifyKey", () => {
    ...
    it("should return false and warn if INTERNAL_ENCRYPTION_KEY length is not 64", () => {
      ...
      expect(verifyKey()).toBe(false);
      expect(consoleWarnMock).toHaveBeenCalledWith(expect.stringContaining("64-character hex string"));
    ...
    it("should return false and log error if encryption verification fails (invalid hex)", () => {
      ...
      expect(verifyKey()).toBe(false);
      expect(consoleErrorMock).toHaveBeenCalledWith(expect.stringContaining("verification failed"), expect.any(String));
    });
  });
