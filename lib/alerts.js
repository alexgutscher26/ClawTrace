 ...
 */
export async function processSmartAlerts(agentId, currentStatus, metrics = {}) {
    try {
        ...
        for (const config of configs) {
            ...
            if (triggered) {
                await dispatchAlert(config, agentId, reason, type, metrics);
            }
        }
    } catch (err) {
...
async function dispatchAlert(config, agentId, reason, type, metrics) {
    try {
        // 1. Get Agent Name
        ...
        // 3. Update cooldown
        await supabaseAdmin.from('alert_configs')
    ...
    await fetch(url, {
        ...
        body: JSON.stringify({
            embeds: [{
                ...
                footer: { text: 'OpenClaw Fleet Monitor' }
            }]
        })
    });
}
