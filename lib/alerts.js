...
/**
 * Process smart alerts based on agent metrics and status against alert configurations.
 *
 * This function fetches alert configurations for a given agent, checks if the agent is in a cooldown period,
 * evaluates the current status and metrics against defined thresholds, and dispatches alerts if necessary.
 * It handles both agent-specific and fleet-wide configurations, ensuring alerts are triggered appropriately
    ...
    for (const config of configs) {
      ...
      if (!triggered && currentStatus !== 'offline') {
        ...
        if (metrics.cpu_usage >= config.cpu_threshold) {
          reasons.push(`CPU usage exceeded threshold: ${metrics.cpu_usage}% (limit: ${config.cpu_threshold}%)`);
        }
        if (metrics.memory_usage >= config.mem_threshold) {
          reasons.push(`Memory usage exceeded threshold: ${metrics.memory_usage}% (limit: ${config.mem_threshold}%)`);
        }
        if (metrics.latency_ms >= config.latency_threshold) {
          reasons.push(`Latency exceeded threshold: ${metrics.latency_ms}ms (limit: ${config.latency_threshold}ms)`);
        }
...
async function dispatchAlert(config, agentId, reason, type, metrics, agentName = null) {
  try {
    ...
    const alertBody = {
      ...
      link: `${process.env.NEXT_PUBLIC_BASE_URL}/agents/${agentId}`,
      severity: type
    };
