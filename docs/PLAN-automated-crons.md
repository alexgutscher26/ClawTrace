...
- **Risk:** Token Timeouts on cleanup.
  - _Mitigation:_ If table is huge, delete in chunks (e.g., `LIMIT 1000`). For now, a simple delete is acceptable for MVP.
- **Risk:** accidental public access.
  - _Mitigation:_ Strict `CRON_SECRET` validation.
- **Risk:** Timezone confusion.
  - _Mitigation:_ Use UTC for all DB comparisons.
